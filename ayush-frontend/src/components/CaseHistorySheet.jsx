...
export default function CaseHistorySheet({ patient, onClose, autoDownload = false }) {
  ...
  const downloadPdf = useCallback(async () => {
    ...
    try {
      ...
      await html2pdf()
        .set({
          margin: 10,
          filename: `VRRTANT_CaseSheet_${token}_${safeName}.pdf`,
  ...
  return (
    <div className="fixed inset-0 z-[300] bg-black/80 flex items-start justify-center overflow-auto p-4" onClick={onClose}>
      ...
      >
        {/* ── Official badge + VRRTANT wordmark (inline light styles: print ignores the screen theme) ── */}
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, marginBottom: 6 }}>
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, border: `1px solid ${BRASS}`, background: '#fffbeb', color: '#92400e', borderRadius: 4, padding: '2px 8px', fontSize: 9.5, fontWeight: 700, letterSpacing: 0.6 }}>
            ✓ {OFFICIAL_BADGE_TEXT}
          </span>
          <span style={{ fontSize: 11, color: MUTED, whiteSpace: 'nowrap' }}>
            <strong style={{ color: INK, letterSpacing: 1.5 }}>{BRAND.name}</strong>{' '}
            <span style={{ color: BRASS, fontFamily: "'Source Serif 4', Georgia, serif" }}>{BRAND.devanagari}</span> · Clinical HealthOS
          </span>
        </div>
        ...
        <Section n={6} title="Physician Signature, Registration & Official Stamp">
          ...
          <div style={{ marginTop: 10, borderTop: `3px double ${EMERALD}`, paddingTop: 4, fontSize: 9.5, color: MUTED, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
            <span>Generated by {BRAND.name} ({BRAND.devanagari}) Clinical HealthOS · {BRAND.taglineEn} · AI findings require physician verification &amp; dual sign-off.</span>
